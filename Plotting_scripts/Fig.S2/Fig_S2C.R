...
setwd('E:/experiment/Ğ£×¼´íÎóÂÊ/ZS97')

# Read the data from the file
data_gatk  <- read.csv('GATK_ZS97_repeattimecount.txt', sep = '\t')
data_hip <- read.csv('hipSTR_ZS97_repeattimecount.txt', sep = '\t')
data_lob <- read.csv('lobSTR_ZS97_repeattimecount.txt', sep = '\t')

x_range <- range(c(data_gatk$refrptime, data_hip$refrptime, data_lob$refrptime))
y_range <- range(c(data_gatk$gatkSTRrepeattime, data_hip$hiprepeattime, data_lob$lobSTRrepeattime))
...
############################################################################################
scatter_plot <- ggplot(data_lob, aes(x = refrptime, y = lobSTRrepeattime)) +
